      ...
      const InferenceParams* params, void* buffers[])
  {
    ...
    for (size_t i = 0; i < params->num_inputs; ++i) {
      ...
      std::vector<int64_t> shape(
          params->dims[i].begin(),
          params->dims[i].begin() + params->num_dims[i]);
    ...
    switch (type) {
      case at::kFloat:
        return torch::from_blob(
                   reinterpret_cast<float*>(ptr), shape, at::kFloat)
            .contiguous();
      case at::kInt:
        return torch::from_blob(
                   reinterpret_cast<int32_t*>(ptr), shape, at::kInt)
            .contiguous();
      case at::kLong:
        return torch::from_blob(
                   reinterpret_cast<int64_t*>(ptr), shape, at::kLong)
            .contiguous();
      case at::kBool:

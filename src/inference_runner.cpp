...
void client_thread(InferenceQueue&      queue,
                   const torch::Tensor& input_ref,
...
void server_thread(InferenceQueue&             queue,
...
void run_inference_loop(const ProgramOptions& opts, StarPUSetup& starpu) {
  ...
  std::thread server([&]() { server_thread(queue, module, starpu, opts, output_ref); });

  std::thread client([&]() { client_thread(queue, input_ref, output_ref, opts.iterations); });

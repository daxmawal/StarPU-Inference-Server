#!/usr/bin/env python3
"""
Plot batch latencies from trace.csv.

This script expects the CSV generated by batching tracing (when trace_enabled is
true). It produces a battery of plots covering latency breakdowns, worker usage,
and (when `metrics.csv` is present) queue size and rejected requests over time.

Usage:
  ./scripts/plot_batch_summary.py PATH_TO/trace.csv
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot batch latencies from trace.csv.")
    parser.add_argument(
        "summary_csv",
        type=Path,
...
def load_queue_metrics(csv_path: Path) -> Tuple[List[float], List[int], List[int]]:
    times_s: List[float] = []
    sizes: List[int] = []
    rejected: List[int] = []
    if not csv_path.exists():
        return times_s, sizes, rejected
    with csv_path.open(newline="") as handle:
        reader = csv.DictReader(handle)
        required = {"timestamp_us", "queue_size", "rejected_total"}
        ...
        for row in reader:
            try:
                ...
                size = int(row["queue_size"])
                rej = int(row["rejected_total"])
            except ValueError as exc:
                raise ValueError(f"Invalid queue metrics row: {row}") from exc
            times_s.append(ts / 1_000_000.0)
            sizes.append(size)
            rejected.append(rej)
    return times_s, sizes, rejected
...
def plot_queue_metrics(
    ...
    sizes: Sequence[int],
    rejected_totals: Sequence[int],
) -> None:
    if not times_s:
        ax.set_title("Queue size & rejections (no data)")
        ax.set_axis_off()
        return
    ...
    if rejected_totals:
        ...
        ax2.step(
            ...
            color="#d62728",
            alpha=0.7,
            label="rejections (cumulative)",
        )
        ax2.set_ylabel("Rejections (cumulative)")
        lines, labels = ax.get_legend_handles_labels()
        lines2, labels2 = ax2.get_legend_handles_labels()
        ax2.legend(lines + lines2, labels + labels2, loc=LEGEND_LOC_UPPER_RIGHT)
    else:
        ax.legend(loc=LEGEND_LOC_UPPER_RIGHT)
    ax.set_title("Queue size and rejected requests over time")
    ax.set_xlabel(TIME_SINCE_FIRST_ARRIVAL_LABEL)
    ax.set_ylabel("Queue size (requests)")
    ax.grid(True, linestyle="--", alpha=0.3)
...
def load_plot_inputs(args: argparse.Namespace) -> PlotInputs | None:
    ...
    queue_csv = csv_path.parent / "metrics.csv"
    try:
        queue_times_s, queue_sizes, rejected_totals = load_queue_metrics(queue_csv)
    except ValueError as exc:
        print(f"warning: {exc}", file=sys.stderr)
        queue_times_s, queue_sizes, rejected_totals = [], [], []
...
def _plot_radar(
    ...
    radar_index = len(axes) - 1
    axes[radar_index].remove()
    axes[radar_index] = fig.add_subplot(
        len(axes), 1, radar_index + 1, projection="polar"
    )
    plot_worker_radar(axes[radar_index], all_workers, all_breakdowns)
...
def render_plots(
    ...
    plot_queue_metrics(axes[34], queue_times_s, queue_sizes, rejected_totals)
...
def create_figure() -> tuple[plt.Figure, list[plt.Axes]]:
    fig, axes_array = plt.subplots(36, 1, figsize=(12, 140), sharex=False)

...
| `trace_file` | Directory for the batching Perfetto trace (requires `trace_enabled: true`). The server writes `batching_trace.json` and `batching_trace_summary.csv` there, warmup batches are excluded from the CSV and plots. | `.` |

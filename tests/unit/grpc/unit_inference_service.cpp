...
TEST_F(
    ...
    ValidateInputsUsesRequestBackedBufferWhenEnabledAndOwnerProvided)
{
  std::vector<float> data = {kF1, kF2, kF3, kF4};
  auto req = starpu_server::make_model_infer_request({
      {{2, 2}, at::kFloat, starpu_server::to_raw_data(data)},
  ...
  const auto* raw_ptr =
      static_cast<const void*>(req.raw_input_contents(0).data());
  EXPECT_EQ(inputs[0].data_ptr(), const_cast<void*>(raw_ptr));
  EXPECT_EQ(keep_alive[0].get(), raw_ptr);
  EXPECT_EQ(inputs[0].data_ptr(), const_cast<void*>(keep_alive[0].get()));
  EXPECT_EQ(
      inputs[0].nbytes(),
      static_cast<int64_t>(req.raw_input_contents(0).size()));
...
TEST_F(
    InferenceServiceTest,
    ValidateInputsCopiesRequestBufferWhenOwnerProvidedButFeatureDisabled)
{
  std::vector<float> data = {kF1, kF2, kF3, kF4};
  auto req = starpu_server::make_model_infer_request({
      {{2, 2}, at::kFloat, starpu_server::to_raw_data(data)},
  ...
  std::vector<std::shared_ptr<const void>> keep_alive;
  auto request_owner = std::make_shared<int>(1);
  auto status = service->validate_and_convert_inputs(
      &req, inputs, &keep_alive, request_owner);
